...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-[var(--editable-shadow)] transition hover:-translate-y-1 hover:shadow-[var(--editable-shadow-strong)] ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
    ...
    <EditableSiteShell>
      <main className="min-h-screen">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.5rem] border border-[var(--editable-border)] bg-white/76 p-6 shadow-[var(--editable-shadow)] backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
            ...
            <form action="/search" className="self-end rounded-[2rem] border border-[var(--editable-border)] bg-[var(--slot4-warm)] p-4 sm:p-5">
              <input type="hidden" name="master" value="1" />
              <label className="flex items-center gap-3 rounded-[1.2rem] border border-[var(--editable-border)] bg-white px-4 py-3">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="flex items-center gap-2 rounded-[1.2rem] border border-[var(--editable-border)] bg-white px-4 py-3">
                ...
                <select name="task" defaultValue={task} className="rounded-[1.2rem] border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-black outline-none">
                  <option value="">All content types</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--slot4-accent-fill)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/article" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-5 py-3 text-sm font-black uppercase tracking-[0.12em]">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
