...
const taskDeck: Record<TaskKey, { icon: typeof FileText; archiveClass: string; promise: string; badge: string }> = {
  article: { icon: FileText, archiveClass: 'grid items-start gap-5 md:grid-cols-2 xl:grid-cols-3', promise: 'Readable editorial cards with room for headlines and excerpts.', badge: 'Read' },
...
export function TaskArchiveView({ task, posts, pagination, category, basePath }: { task: TaskKey; posts: SitePost[]; pagination: SiteFeedPagination; category: string; basePath: string }) {
  ...
  const archivePosts =
    isArticle && posts.length > 6
      ? (() => {
          const list = [...posts]
          const promoted = list.splice(6, 1)[0]
          if (promoted) list.splice(2, 0, promoted)
          return list
        })()
      : posts
  ...
  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className={`mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-12 sm:px-6 lg:px-8 lg:py-20 ${isArticle ? 'lg:grid-cols-[1.15fr_1fr] lg:items-stretch' : 'lg:grid-cols-[1.05fr_0.95fr]'}`}>
          <div className={`border border-[var(--editable-border)] bg-[var(--archive-surface)] shadow-[0_24px_80px_rgba(15,23,42,0.08)] ${isArticle ? 'rounded-[2.1rem] p-8 sm:p-10' : 'rounded-[2.5rem] p-7 sm:p-10'}`}>
            <div className={`inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white/70 text-xs font-black uppercase tracking-[0.24em] text-[var(--archive-accent)] ${isArticle ? 'px-4 py-2' : 'px-4 py-2'}`}><Icon className="h-4 w-4" /> {label}</div>
            <h1 className={`${isArticle ? 'mt-5 max-w-3xl text-[4rem] font-black leading-[0.95] tracking-[-0.07em] sm:text-[4.3rem]' : 'mt-5 max-w-4xl text-5xl font-black leading-[0.95] tracking-[-0.07em] sm:text-6xl'}`}>{voice?.headline || `Browse ${label}`}</h1>
            <p className={`mt-6 max-w-2xl text-base ${isArticle ? 'leading-8 opacity-72' : 'leading-8 opacity-70'}`}>{voice?.description || SITE_CONFIG.description}</p>
            <div className={`mt-6 rounded-[1.2rem] border border-[var(--editable-border)] bg-white/55 p-4 text-sm font-bold leading-7 opacity-75 ${isArticle ? 'max-w-xl' : ''}`}>{deck.promise}</div>
          ...
          <div className={`border border-[var(--editable-border)] bg-white/70 p-5 shadow-sm backdrop-blur ${isArticle ? 'rounded-[1.8rem] flex h-full flex-col' : 'rounded-[2rem]'}`}>
            {isArticle ? (
              <>
                <div className="rounded-[1rem] border border-[var(--editable-border)] bg-white p-4">
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--archive-accent)]">Article layout</p>
                  <p className="mt-2 text-sm leading-6 opacity-70">
                    Browse by category, then scan the featured story first and continue through the compact editorial feed.
                  </p>
                </div>
                <form action={basePath} className="mt-4 rounded-[1rem] border border-[var(--editable-border)] bg-white p-4">
                  ...
                  <button className="mt-3 h-12 w-full rounded-[1rem] bg-[var(--archive-text)] text-sm font-black text-[var(--archive-bg)]">Apply</button>
                  <p className="mt-3 text-xs font-bold opacity-55">Showing: {categoryLabel}</p>
                </form>
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div className="rounded-[0.9rem] border border-[var(--editable-border)] bg-white p-3">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] opacity-55">Layout</p>
                    <p className="mt-1 text-sm font-bold">Editorial</p>
                  </div>
                  <div className="rounded-[0.9rem] border border-[var(--editable-border)] bg-white p-3">
                    <p className="text-[10px] font-black uppercase tracking-[0.16em] opacity-55">Category</p>
              ...
              <form action={basePath}>
                <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] opacity-55"><Filter className="h-4 w-4" /> Filter</div>
                <select name="category" defaultValue={category} className={`mt-4 h-12 w-full border border-[var(--editable-border)] bg-white px-4 text-sm font-bold outline-none ${isArticle ? 'rounded-[1rem]' : 'rounded-2xl'}`}>
                  <option value="all">All categories</option>
                  {CATEGORY_OPTIONS.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
                </select>
                <button className={`mt-3 h-12 w-full bg-[var(--archive-text)] text-sm font-black text-[var(--archive-bg)] ${isArticle ? 'rounded-[1rem]' : 'rounded-2xl'}`}>Apply</button>
                <p className="mt-3 text-xs font-bold opacity-55">Showing: {categoryLabel}</p>
              </form>
            )}
          </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 pb-16 sm:px-6 lg:px-8">
          {posts.length ? (
            <div className={deck.archiveClass}>
              {archivePosts.map((post, index) => <ArchivePostCard key={`${post.id || post.slug}-${index}`} post={post} task={task} basePath={basePath} index={index} />)}
            </div>
          ) : (
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  if (index === 0) {
    return (
      <Link href={href} className="group self-start md:col-span-2 xl:col-span-3 overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
        <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative min-h-[300px] overflow-hidden bg-black/5">
            <img src={image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
            <span className="absolute left-4 top-4 rounded-full bg-[var(--archive-text)] px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--archive-bg)]">Featured Story</span>
          </div>
          <div className="p-7 sm:p-9">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--archive-accent)]">{category}</p>
            <h2 className="mt-3 font-serif text-4xl font-bold leading-tight tracking-tight">{post.title}</h2>
            <p className="mt-4 line-clamp-4 text-sm leading-7 opacity-70">{getSummary(post)}</p>
            <span className="mt-6 inline-flex items-center gap-2 text-sm font-black">Read full article <ArrowRight className="h-4 w-4" /></span>
          </div>
        </div>
      </Link>
    )
  ...
  if (index % 4 === 0 || index === 2) {
    return (
      <Link href={href} className="group self-start overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl md:col-span-2">
  ...
  return (
    <Link href={href} className="group self-start overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
