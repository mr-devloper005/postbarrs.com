...
const hardenLinks = (html: string) =>
  html.replace(/<a\s+([^>]*href=["'][^"']+["'][^>]*)>/gi, (_match, attrs) => {
    let next = String(attrs)
      .replace(/\s+on\w+=("[^"]*"|'[^']*'|[^\s>]+)/gi, '')
      .replace(/\s+(style|class|id|width|height|align)=("[^"]*"|'[^']*'|[^\s>]+)/gi, '')
...
const estimateReadMinutes = (post: SitePost) => {
  const text = `${post.title} ${summaryText(post)} ${getBody(post)}`.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()
  const words = text ? text.split(' ').length : 0
  return Math.max(1, Math.round(words / 220))
}

const getTagList = (post: SitePost) => (Array.isArray(post.tags) ? post.tags.filter((tag): tag is string => typeof tag === 'string' && Boolean(tag.trim())) : []).slice(0, 4)
...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-4 py-2 text-sm font-black uppercase tracking-[0.12em] shadow-sm">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  const publishedLabel = formatDetailDate(post.publishedAt)
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[2.5rem] border border-[var(--editable-border)] bg-white p-6 shadow-[var(--editable-shadow)] sm:p-9">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-[var(--editable-shadow)]">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.5rem] border border-[var(--editable-border)] bg-white p-7 shadow-[var(--editable-shadow)] sm:p-10">
        <BackLink task="sbm" />
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[2.5rem] border border-[var(--editable-border)] bg-white p-8 text-center shadow-[var(--editable-shadow)] lg:sticky lg:top-24 lg:self-start">
...
function BodyContent({ post, compact = false }: { post: SitePost; compact?: boolean }) {
  return <div className={`article-content max-w-none ${compact ? 'text-base leading-8' : 'text-lg leading-9'} opacity-80`} dangerouslySetInnerHTML={{ __html: formatPlainText(getBody(post)) }} />
}
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-[var(--editable-border)] bg-white shadow-[var(--editable-shadow)]">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-[var(--editable-border)] bg-white p-5 shadow-[var(--editable-shadow)]">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      ...
      {related.length ? (
        <div className="rounded-[2rem] border border-[var(--editable-border)] bg-white/78 p-5 shadow-[var(--editable-shadow)] backdrop-blur">
